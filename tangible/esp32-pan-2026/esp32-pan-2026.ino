...
void setup() {
  ...
  delay(1000);
  Serial.println();
  Serial.println("ESP32 pan WebSocket streamer");

  if (!sensorCodeBegin()) {
    Serial.println("Sensor init failed. Rebooting in 10 s...");
    delay(10000);
    ESP.restart();
  }
...
void loop() {
  ...
  if (millis() - lastSend > PAN_SEND_INTERVAL_MS) {
    lastSend = millis();
    const float heading = sensorCodeGetHeading();
